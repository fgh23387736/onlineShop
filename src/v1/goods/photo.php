<?php
	...
	*/
	...
	function toPost($request_data){
		...
		$imgs=new Imgs();
		$mypdo=new MySqlPDO();
		$sql="SELECT `Id` FROM `os_goods` WHERE `Id`=? AND `UserId`=?";
		$mypdo->prepare($sql);
		if ($mypdo->executeArr(array($request_data['GoodsId'],$_SESSION['id']))) {
			if($rs=$mypdo->fetch()){

			}else{
				https(401);
				echo json_encode(array('Error' => "您不具有权限操作请更换账号"));
	    		exit();
			}
		}
		$request_data['Url']=$imgs->saveBase64toImg('/onlineShop/uploaddate/goods/photos',$request_data['Url']);
		$sql='INSERT INTO `os_goods_photo`
			(
				`GoodsId`,
				`Url`
			) VALUES (
	...
	function toDelete($request_data){
		...
		$sql_f="DELETE FROM `os_goods_photo` WHERE ";
		$sql_b="";
		$wherevalues=array();
		switch ($request_data['Type']) {
			case '0':{
				if(isset($request_data['Search']['Id'])&&!empty($request_data['Search']['Id'])){
					...
					$sql_b.=") AND `GoodsId` IN (SELECT `Id` FROM `os_goods` WHERE `UserId`=?)";
		...
		$sql="SELECT `Url` FROM `os_goods_photo` WHERE ".$sql_b;
		$mypdo->prepare($sql);
	...
	function toGet($request_data){
		...
		$sql_f="SELECT * FROM `os_goods_photo` WHERE ";
		$sql_c="SELECT count(*) AS `AllNumber` FROM `os_goods_photo` WHERE ";
		$sql_b="";
		$wherevalues=array();
		switch ($request_data['Type']) {
			case '0':{
